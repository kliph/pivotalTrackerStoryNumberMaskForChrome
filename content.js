...
function appendStoryNumberWithStoryName_Blocked() {
  if (document.querySelectorAll("[id^='panel_blockers']")[0]) {
    var totalBlockedStoryCount = document
      .querySelectorAll("[id^='panel_blockers']")[0]
      .getElementsByClassName("story model item").length;
...
function appendStoryNumberWithStoryName_MyWork() {
  if (document.querySelectorAll("[id^='panel_my_work']")[0]) {
    var totalMyWorkStoryCount = document
...
function appendStoryNumberWithStoryName_Epic() {
  if (document.querySelectorAll("[id^='panel_epic']")[0]) {
    var totalEpicStoryCount = document
      .querySelectorAll("[id^='panel_epic']")[0]
      .getElementsByClassName("epic model item").length;
...
function appendStoryNumberWithStoryName_Epic_Story() {
  ...
  if (document.querySelectorAll("[id^='panel_epic']")[1]) {
    for (j = 1; j <= epicPanelCount - 1; j++) {
      var totalEpicStoriesCount = document
        .querySelectorAll("[id^='panel_epic']")
        [j].getElementsByClassName("story model item").length;
      if (totalEpicStoriesCount) {
        for (var i = 0; i < totalEpicStoriesCount; i++) {
          var epicStoriesNumber = document
            .querySelectorAll("[id^='panel_epic']")
            [j].getElementsByClassName("story model item")
            [i].getAttribute("data-id");
          if (epicStoriesNumber) {
            var epicStoriesNumberToDisplay = epicStoriesNumber.slice(
              -storyNumberToDisplay_Last_N_Digits
            );
            if (
              document
              ...
              if (!regChecker.test(epicStoriesName)) {
                document
                  ...
                  [i].getElementsByTagName("span")[4]
                  .getElementsByClassName("tracker_markup")[0].innerText =
                  epicStoriesNumberToDisplay + " - " + epicStoriesName;
              }
            }
          }
        }
...
function appendStoryNumberWithStoryName_Search_Label() {
  if (document.querySelectorAll("[id^='panel_search']")[0]) {
    var totalSearchStoryCount = document
